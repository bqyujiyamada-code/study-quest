...
export default function CubeQuest() {
  ...
  const styles: { [key: string]: React.CSSProperties } = {
    ...
    sidebar: {
      width: isMobile ? "100%" : "300px", padding: "16px",
      background: "#0f172a", borderBottom: isMobile ? "1px solid #334" : "none"
    }
  };
  ...
  return (
    <div style={styles.container}>
      <div style={styles.sidebar}>
        <h2 style={{ fontSize: "16px" }}>展開図実験室</h2>
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
          {Object.keys(patterns).map(k => (
            <button key={k} onClick={() => setKey(k)} 
              style={{ padding: "8px", background: key === k ? "#22d3ee" : "#1e293b", border: "none", borderRadius: "6px", color: "white", cursor: "pointer" }}>
              {k === "p01" ? "十字型" : "階段型"}
            </button>
          ))}
        </div>
        <input type="range" min="0" max="100" value={progress} 
          onChange={(e) => setProgress(Number(e.target.value))} style={{ width: "100%", marginTop: "20px" }} />
      </div>
      <div style={{ flex: 1 }}>

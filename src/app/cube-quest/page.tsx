...
export default function CubeQuest() {
  ...
  const styles: { [key: string]: React.CSSProperties } = {
    container: {
      ...
      background: "#020617", color: "white", fontFamily: "sans-serif", overflow: "hidden"
    },
    sidebar: {
      width: isMobile ? "100%" : "300px", flex: isMobile ? "none" : "0 0 300px",
      padding: "16px", background: "#0f172a", boxSizing: "border-box",
      borderBottom: isMobile ? "1px solid #334" : "none",
      borderRight: isMobile ? "none" : "1px solid #334", overflowY: "auto"
    },
      ...
      <div style={styles.sidebar}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
          {Object.keys(patterns).map(k => (
            <button key={k} onClick={() => {setKey(k); setProgress(0);}} style={styles.button(key === k)}>
              {k === "p01" ? "十字型" : "階段型"}
            </button>
          ))}
        </div>
        <input type="range" min="0" max="100" value={progress} 
          onChange={(e) => setProgress(Number(e.target.value))} style={{ width: "100%", marginTop: "24px" }} />
      </div>
